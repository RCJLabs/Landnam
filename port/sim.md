...
**What does NOT collide, and it is worth knowing which:** every file in
...
This was got wrong once, in the direction of caution — the file was renamed
to avoid a redefinition that could not happen — and the rename was reverted
once a two-file experiment showed it compiled. All six collisions above are
real and were proved by compiling the actual files; this seventh was not,
...
## The two branches, merged — and one algorithm that exists twice

The Unreal repo had two lines of work that had never met: the island
worldgen with `LandnamNoise` and `BP_HexGrid` on one branch, this rules port
on the other, both forked from `master`. They are one branch now.

**The single conflict was `golden.json`, and it was the one worth having.**
The worldgen side had added a `worldgen` section — terrain codes, 64 noise
samples, 8 whole islands — to `Content/Data/golden.json`, the UNREAL COPY.
`port/golden.json`, which owns it, had never seen them. That is precisely
the hole `test/goldenport.test.ts` opens by describing:

> *"the vectors were generated by a script that lived on one machine and was
> never committed HERE... A green parity test that cannot see one side of
> the parity is worse than none, because it is reassuring."*

Reopened one directory over, by the same project, six days later.

The vectors are TRUE. All 8 worlds and all 64 samples reproduce exactly from
the live TypeScript — once you know the noise probe is built from a bare
`makeRng(seed).derive(label)` and **not** from `stream(seed, 'worldgen')`,
which is the one thing about them that is not obvious and is now written
down at the call site. So nothing had diverged; they simply were not owned.
They are owned now, and `goldenport.test.ts` recomputes every one.

### Two worldgens, and how they are held together
...
They agree today because both were checked against the same eight worlds.
Nothing kept them agreeing, and the project's own doctrine says what that
costs — `LandnamCanon.h` was written after three copies of `CanonicalString`
drifted, and a fourth was found still drifting a week later.

So both are now pinned to the SAME vectors: the editor's `Landnam.Parity`
checks the UE one against the eight golden worlds, and `Tools/run-parity.sh`
checks the sim one against them, hex for hex, on Linux and in CI on every
push. Neither can drift from the vectors without going red, so neither can
drift from the other in silence. The harness prints terrain by NAME and node
maps the one-letter codes out of `golden.json`, because those codes are
content — retyping them into C++ is the habit the generators exist to
prevent.

**That is a guard, not a fix.** The fix is for the UE one to become a thin
ADAPTER over the sim one: keep `FWorldTile`, `FLandnamWorld` and the four
`UFUNCTION` signatures exactly as Blueprints see them, replace only the
bodies. It needs two small things first — a seed-and-state accessor on
`ULandnamRng`, since `FSimRng::Derive` works off the seed rather than the
state, and `Sim::GenerateWorld` reporting how many attempts it took, so the
adapter does not have to invent `Attempts` and `bValid`.
