...
const { Hex, Rng, Noise, Worldgen } = await loadGame({
  Hex: './src/hex/index.ts',
  Rng: './src/rng.ts',
  Noise: './src/sim/noise.ts',
...
// Noise on its own, so a coastline mismatch can be blamed on the right layer.
// Sampled off the integer lattice and at negative coordinates, where `Math.floor`
// and a C++ cast disagree and the table wrap has to handle a negative index.
const NOISE_SEED = 'noise-probe';
const NOISE_OCTAVES = 5;
const noiseSamples = (() => {
  const fbm = Noise.makeFbm(Rng.makeRng(NOISE_SEED).derive('elevation'), NOISE_OCTAVES);
  const out = [];
  for (let i = 0; i < 64; i++) {
    const x = i * 3.7 - 40.25;
    const y = 90.5 - i * 1.31;
    out.push({ x, y, v: fbm(x, y) });
  }
  return out;
})();

/** One char per terrain, so a whole 52x36 map is a readable 1,872-character line. */
const TERRAIN_CODE = {
  ocean: 'o', shore: 's', meadow: 'm', forest: 'f',
  hills: 'h', mountains: 'M', bog: 'b', valley: 'v',
};

// Eight seeds, not one or two. The terrain codes are quantised, so a map only
// catches a wrong constant when the change moves some hex across a threshold —
// with three maps, nudging SEA_LEVEL by 1e-4 changed nothing anywhere and the
// suite stayed green. More maps means more hexes sitting near every boundary.
// Row-major order matches the C++ tile array exactly.
const WORLD_SEEDS = [
  'raven-skerry-317', 'grim-fjord-101', 'storm-holm-742', 'ash-dale-004',
  'iron-ness-888', 'wolf-strand-256', 'frost-mark-013', 'amber-sund-497',
];
const worldCases = WORLD_SEEDS.map((seed) => {
  const world = Worldgen.generateWorld(Rng.stream(seed, 'worldgen'));

  let terrain = '';
  let rivers = '';
  for (let row = 0; row < world.height; row++) {
    for (let col = 0; col < world.width; col++) {
      const tile = world.tiles[Hex.key(Hex.offsetToAxial(col, row))];
      const code = TERRAIN_CODE[tile.terrain];
      if (!code) throw new Error(`no code for terrain "${tile.terrain}"`);
      terrain += code;
  ...
  return {
    ...
    width: world.width,
    height: world.height,
    landing: { q: world.landing.q, r: world.landing.r },
    terrain,
    rivers,
  };
});
...
const golden = {
  note: 'Generated by ue-port/tools/golden.mjs from src/hex, src/rng and src/sim. Do not hand-edit.',
  ...
  worldgen: {
    terrainCodes: TERRAIN_CODE,
    noise: { seed: NOISE_SEED, label: 'elevation', octaves: NOISE_OCTAVES, samples: noiseSamples },
    worlds: worldCases,
  },
};
...
console.log(`  worldgen: ${noiseSamples.length} noise samples, ${worldCases.length} worlds x ${worldCases[0].terrain.length} hexes`);
